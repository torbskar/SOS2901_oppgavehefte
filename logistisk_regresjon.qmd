...
library(mgcv)        # for GAM (generaliserte additive modeller)
library(pROC)        # Beregne ROC-curve
library(caret)       # Funksjonen confusionMatrix()
```

Før vi går over til logistisk regresjon tar vi en titt på noen utvidelser av lineær regresjon som er nyttige for prediksjon: splines og GAM.

## Splines

I forrige kapittel brukte vi lineær regresjon og så at modellen antar en lineær sammenheng mellom prediktor og utfall. Men hva hvis sammenhengen *ikke* er lineær? En mulighet er å bruke *splines*, som er stykkevis definerte funksjoner som kan tilpasse mer fleksible former.
...
Først leser vi inn datasettet. Deretter kan vi se på innholdet med `skim()`:
...
Vi kan sammenligne lineær og logistisk modell for å se forskjellen i predikert sannsynlighet:
