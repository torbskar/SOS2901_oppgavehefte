...
Hvis du nå lurer på om det spiller noen rolle om du har observert eller predikert i rader/kollonner, så gjør det ikke det. Det er bare to variable krysset mot hverandre. I dette tilfellet er det altså `r tab[2,2]` personer som er gjettet riktig at vil slutte, men bare `r tab[2,2]` av `r sum(tab[2,])` av de som faktisk sluttet ble fanget opp. Det var `r sum(tab[1,2])` som ble feilaktig gjettet at ville slutte, men som altså ikke gjorde det. 
